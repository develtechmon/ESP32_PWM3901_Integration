...
// ===== Barometric Altitude Hold Variables (MS5611/BMP280 style) =====
int16_t baroHistTab[21];        // Circular buffer for 21 pressure readings
int8_t baroHistIdx = 0;         // Index for circular buffer
int32_t baroHigh = 0;           // Running sum of pressure readings
float baroGroundPressure = 0;   // Ground reference pressure
bool altitudeHoldActive = false; // Altitude hold state
bool baroRead = false;          // State machine for pressure reading

// Altitude PID variables (using same gains as reference)
float pid_p_gain_alt = 0.6;     // Proportional gain
float pid_i_gain_alt = 0.8;     // Integral gain  
float pid_d_gain_alt = 5.0;     // Derivative gain
int pid_max_alt = 200;          // Maximum PID output (increased for more authority)

float pid_alt_setpoint = 0;     // Target altitude (pressure)
float pid_alt_input = 0;        // Current altitude (filtered pressure)
...
float currentPressure = 0;      // Current pressure reading
unsigned long lastBaroTime = 0; // Timing for pressure readings
...
// ===== Barometric Altitude Hold Functions =====

// Initialize barometer and calibrate ground pressure
void initializeBarometricAltitude() {
  Serial.println("🌡️ Calibrating barometric altitude hold...");
  ...
  for (int cal_int = 0; cal_int < 150; cal_int++) {
    // Read pressure from BMP280 (you already have this function)
    barometer_signals();
    currentPressure = AltitudeBarometer;
    
    // Store in circular buffer for averaging  
    baroHistTab[baroHistIdx] = currentPressure / 10;  // Scale like original
    ...
    baroGroundPressure = baroGroundPressure * 0.7f + baroHigh * 0.15f;
  ...
  Serial.println(" ✅ Barometric calibration complete!");
  Serial.print("Ground pressure reference: ");
  Serial.println(baroGroundPressure);
}

// Update barometric altitude readings (call this every loop)
void updateBarometricAltitude() {
  // Read pressure at reasonable intervals (not every loop for efficiency)
  if (millis() - lastBaroTime >= 20) { // 50Hz update rate
    lastBaroTime = millis();
    ...
    baroHistTab[baroHistIdx] = currentPressure / 10;
    baroHigh += baroHistTab[baroHistIdx];              // Add new reading
    baroHigh -= baroHistTab[(baroHistIdx + 1) % 21];  // Remove oldest reading
    baroHistIdx++;
    if (baroHistIdx == 21) baroHistIdx = 0;
    ...
    if (!altitudeHoldActive) {
      ...
      pid_alt_setpoint = pid_alt_setpoint * 0.7f + baroHigh * 0.15f;
      pid_alt_input = pid_alt_setpoint;
    } else {
      // When in altitude hold, filter current reading
      pid_alt_input = pid_alt_input * 0.7f + baroHigh * 0.15f;
    }
  }
}

// Calculate altitude PID (exactly like reference code)
void calculateBarometricAltitudePID() {
  if (!altitudeHoldActive) {
    pid_output_alt = 0;
    return;
  }
...
void handleAltitudeHoldSwitching() {
  // Check channel 6 for altitude hold switch (adjust channel as needed)
  bool altHoldSwitch = (channelValues[5] > 1700); // Channel 6 high = altitude hold on
  
  // Deactivate altitude hold
  if (!altHoldSwitch && altitudeHoldActive) {
    altitudeHoldActive = false;
    pid_output_alt = 0;
    Serial.println("🔓 Barometric Altitude Hold DEACTIVATED");
  }
  ...
  if (altHoldSwitch && !altitudeHoldActive) {
    ...
    Serial.println("🔒 Barometric Altitude Hold ACTIVATED");
    Serial.print("Target pressure: ");
    Serial.print(pid_alt_setpoint);
    Serial.print(" | Baseline throttle: ");
...
void setup() {
  ...
  // Initialize barometric altitude hold
  initializeBarometricAltitude();
...
void loop() {
  ...
  // Update barometric altitude tracking
  updateBarometricAltitude();
  
  // Handle altitude hold switching logic
  handleAltitudeHoldSwitching();
  
  // Check for VL53L0X altitude hold mode (channel 6) - for close-range indoor use
  if (channelValues[5] > 1700 && !altitudeHoldActive) { // Only if barometric hold is off
    if (!altitudeHoldMode) {
      altitudeHoldMode = true;
      targetAltitude = currentAltitude;
      baseThrottle = channelValues[2];
      integralAlt = 0;
  ...
  // Calculate barometric altitude PID
  calculateBarometricAltitudePID();
  ...
  if (ReceiverValue[2] < 1030) {
    ...
    PrevErrorAngleRoll = PrevErrorAnglePitch = 0;    
    PrevItermAngleRoll = PrevItermAnglePitch = 0;
    // Reset ALL altitude hold integrators
    integralX = integralY = integralAlt = 0;
    pid_i_mem_alt = 0; // Reset barometric altitude integral too
  }
