...
#define FLOW_SCK_PIN   18   // SPI Clock
#define FLOW_MISO_PIN  19   // SPI MISO  
#define FLOW_MOSI_PIN  23   // SPI MOSI

// ===== PWM3901 Registers - like addresses in a filing cabinet =====
#define MOTION_REG    0x02
#define X_LOW_REG     0x03
#define X_HIGH_REG    0x04
#define Y_LOW_REG     0x05
#define Y_HIGH_REG    0x06
...
// ===== PWM3901 Helper Functions - like translating between languages =====
byte askFlowSensor(byte question) {
  digitalWrite(FLOW_CS_PIN, LOW);   // "Hey sensor, listen!"
  ...
  digitalWrite(FLOW_CS_PIN, HIGH);  // "Thanks, bye!"
  delayMicroseconds(50);
  return answer;
}

// Read optical flow - like asking "which way did I move?"
void readOpticalFlow(int16_t* deltaX, int16_t* deltaY) {
  // Check if there's motion
  byte motionReg = askFlowSensor(MOTION_REG);
  bool hasMotion = (motionReg & 0x80) != 0;
  
  if (hasMotion) {
    // Get movement data - like putting together puzzle pieces
    byte xLow = askFlowSensor(X_LOW_REG);
    byte xHigh = askFlowSensor(X_HIGH_REG);
    byte yLow = askFlowSensor(Y_LOW_REG);
    byte yHigh = askFlowSensor(Y_HIGH_REG);
    
    *deltaX = (int16_t)((xHigh << 8) | xLow);
    *deltaY = (int16_t)((yHigh << 8) | yLow);
  } else {
    *deltaX = 0;
    *deltaY = 0;
  }
...
void initializePositionSensors() {
  ...
  // Initialize PWM3901 optical flow sensor
...
void updatePosition() {
  // Read optical flow movement
  int16_t deltaX, deltaY;
  readOpticalFlow(&deltaX, &deltaY);
